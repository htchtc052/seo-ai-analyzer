...
export function ExampleTopic({ topics, topic, disabled, onSelect }: Props) {
  return <p className="flex items-center gap-1 text-xs text-muted-foreground">
    Example:
    <Button type="button" variant="link" size="xs" disabled={disabled} onClick={() => onSelect(topic)}>{topic.title}</Button>
    <Dialog>
