...
export function ExampleTopic({ topics, topic, disabled, onSelect }: Props) {
  return (
    <p className="flex items-center gap-1 text-xs text-muted-foreground">
      Example:
      <Button type="button" variant="link" size="xs" disabled={disabled} onClick={() => onSelect(topic)}>
        {topic.title}
      </Button>
      <Dialog>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Links for your article and both competitors are filled in from the chosen topic.
            </DialogDescription>
          </DialogHeader>
              ...
              <DialogClose key={item.title} asChild>
                ...
                >
                  ...
                  <span className="font-normal text-muted-foreground">
                    {[item.article, ...item.competitors].map((example) => example.site).join(' · ')}
                  </span>
                </Button>
              </DialogClose>
            ))}
