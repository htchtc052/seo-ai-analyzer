...
export function useAnalysisRuns() {
  return useQuery({
    ...
    select: (runs) => runs.map((run) => ({ run, status: getRecommendationStatus(run.competitorCount, run) })),
    refetchIntervalInBackground: true,
    refetchInterval: (query) =>
      query.state.data?.some((run) => getRecommendationStatus(run.competitorCount, run) === 'pending')
        ? POLL_INTERVAL_MS
        : false,
  });
}
