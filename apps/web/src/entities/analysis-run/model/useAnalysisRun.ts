...
export function useAnalysisRun(id: string) {
  return useQuery({
    ...
    select: (run) => ({ run, status: getRecommendationStatus(run.competitors.length, run) }),
    refetchIntervalInBackground: true,
    refetchInterval: (query) => {
      const run = query.state.data;
      return run && getRecommendationStatus(run.competitors.length, run) === 'pending' ? POLL_INTERVAL_MS : false;
    },
  });
}
