...
export function ScoreChart({ fragments }: { fragments: FragmentScore[] }) {
  ...
  return (
    <figure className="grid gap-6">
      <figcaption>
        <Muted>Cosine similarity of each paragraph to the query, on a 0–1 scale</Muted>
      </figcaption>
        ...
        <section key={sectionIndex} className="grid gap-3">
          ...
          <ol className="grid gap-4">
            {section.fragments.map((fragment, index) => (
              <li key={index} className="grid grid-cols-[1fr_3rem] items-center gap-x-3 gap-y-1">
