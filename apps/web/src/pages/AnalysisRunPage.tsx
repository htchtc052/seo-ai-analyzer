    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-lg">
          <ExternalLink href={run.article.sourceUrl}>{run.article.title}</ExternalLink>
        </CardTitle>
        <CardAction>
      ...
      <CardContent className="grid gap-8">
        ...
        {recommendationStatus === 'failed' && (
          <Alert variant="destructive">
            <AlertDescription>
              {run.recommendationJob?.failedReason ?? 'The recommendation job is no longer available.'}
            </AlertDescription>
          </Alert>
        )}
        {recommendationStatus === 'ready' && (
          <>
            {run.missingEntities.length > 0 && (
              <TextList title="Suggested missing topics" items={run.missingEntities} />
            )}
