...
export class WebPageService {
  ...
  private async fetchHtml(url: string): Promise<string> {
    const response = await fetch(url, { signal: AbortSignal.timeout(TIMEOUT_MS) }).catch(loadFailed);
    if (response.status !== 200) throw new PageFetchException(`The page responded with HTTP ${response.status}`);

    const contentType = response.headers.get('content-type') ?? '';
    if (!/^(text\/html|application\/xhtml\+xml)\b/i.test(contentType)) {
      throw new PageFetchException(`Expected an HTML page, got ${contentType || 'no content type'}`);
    }

    const html = await response.text().catch(loadFailed);
    if (html.length > MAX_LENGTH) throw new PageFetchException('The page is larger than 5 MB');
    return html;
  }
}
