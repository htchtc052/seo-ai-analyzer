...
const articleRefSchema = articleSchema.pick({ id: true, sourceUrl: true, title: true });
...
export const startAnalysisSchema = z.object({
  ...
  competitorIds: z
    ...
    .max(2)
    .refine((ids) => new Set(ids).size === ids.length, 'competitorIds must be unique'),
...
export const featuresResponseSchema = z.object({ features: z.object({ recommendations: z.boolean() }) });
