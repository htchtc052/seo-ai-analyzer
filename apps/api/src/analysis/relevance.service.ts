...
export class RelevanceService {
  ...
  async score(query: string, sections: ArticleSection[]): Promise<number[]> {
    ...
    const [queryEmbedding, ...embeddings] = await this.llm.embed([query, ...texts]);
    return embeddings.map((embedding) => cosineSimilarity(queryEmbedding!, embedding));
  }

  scoreFragments(sections: ArticleSection[], scores: number[]): FragmentScore[] {
    return toFragments(sections).map((fragment, index) => ({ ...fragment, score: scores[index]! }));
  }
